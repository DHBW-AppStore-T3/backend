"""Parsing of an app repository's HCL variable declarations.

Reads ``terraform/variables.tf`` and ``packer/variables.pkr.hcl`` out of
a cloned app repo and turns each ``variable "name" {...}`` block into the
dict shape the deployment wizard renders. That includes the
``@openstack:`` marker grammar documented below, the ``varScope``
(all / team / user) rules, and the HCL-type validation that goes with
them.

This used to live inline in ``app.routers.apps``, which made that module
1748 lines of which the first route appeared at line 1315. Nothing here
touches FastAPI, the database or the ORM - it is pure text in, dicts
out - so it belongs beside the other services rather than in a router.

The public surface is :func:`parse_terraform_variables` and
:func:`parse_packer_variables`; everything else is an implementation
detail that the unit tests reach into directly.
"""
